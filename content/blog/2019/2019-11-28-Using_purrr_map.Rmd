...
either by hand (7x copy/paste) or with `purrr::map()`!
...
```{r}
ggplot(cant_level, aes(x = long, y = lat, group = group)) +
  geom_polygon(aes(fill = factor(region)), color = "white", size = 0.2) +
  scale_fill_manual(name = "Region",
                    values = c("Ticino" = "grey90",
                               "Romandie" = "#b2df8a",
                               "Deutschschweiz" = "#a6cee3")) +
  ...
  geom_polygon(aes(x = long, y = lat, group = group), 
               data = shp_df[["g1s15"]], fill = "blue", color = NA)
```
